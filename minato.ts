...
function isInsideStartalePreview(connectorLabel: string): boolean {
  if (typeof window === 'undefined' || window.parent === window) return false

  const referrer = typeof document === 'undefined' ? '' : document.referrer.toLowerCase()
  return connectorLabel.includes('startale') || referrer.includes('app.startale.com')
}
...
export function useMinatoNetwork() {
  ...
  const isEmbeddedStartale = isInsideStartalePreview(connectorLabel)

  // Only probe ordinary browser wallets. Startale Preview's hosted provider can
  // reject or throw during eth_chainId probing, so embedded Preview is treated
  // as explicitly read-only while Mandate remains deployed on Minato.
  useEffect(() => {
    if (!isConnected || !connector || isEmbeddedStartale) {
  ...
  }, [connector, isConnected, isEmbeddedStartale, reportedChainId])

  const effectiveChainId = isEmbeddedStartale
    ? reportedChainId
    : providerChainId ?? reportedChainId

  const isProviderChainKnown =
    isEmbeddedStartale || !isConnected || providerChainId !== undefined || !isResolvingProviderChain

  const isCorrectChain =
    !isConnected ||
    (!isEmbeddedStartale &&
      isProviderChainKnown &&
      effectiveChainId === MINATO_CHAIN_ID)
  ...
  const writeBlockedReason = useMemo(() => {
    ...
    if (isEmbeddedStartale) {
      return (
        "Startale Preview currently prepares hosted wallet transactions on Soneium Mainnet, " +
        "while this Mandate deployment is on Soneium Minato. Preview is read-only; no transaction can be prepared."
      )
    }

    if (isResolvingProviderChain) {
      return 'Checking the connected wallet network. No transaction has been prepared.'
    }
  ...
  async function switchToMinato(): Promise<{ ok: boolean; message?: string }> {
    ...
    if (isEmbeddedStartale) return { ok: false, message: writeBlockedReason }
